    ...
    <div class="rounded-xl border bg-card shadow-sm">
        <table class="w-full">
            ...
            <tbody>
                    ...
                    <td class="p-4 text-right">
                        <form method="POST" action="{{ route('admin.categories.update', $cat) }}"
                            ...
                            <input type="text" name="name" value="{{ $cat->name }}"
                                class="h-8 w-40 rounded-md border border-input bg-transparent px-2 text-sm">
                            <button type="submit"
                ...
                @empty
                <tr>
                    <td colspan="3" class="p-8 text-center text-sm text-muted-foreground">No categories yet.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
