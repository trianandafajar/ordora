...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <button data-modal="create-kasir" class="rounded-md bg-primary text-primary-foreground text-sm font-medium h-9 px-4 hover:opacity-90 transition-opacity">
            Add Kasir
        </button>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody>
                @forelse($kasirs as $kasir)
                    <tr class="border-t last:border-0 hover:bg-accent/30">
                        ...
                        <td class="p-4 text-right">
                            <form method="POST" action="{{ route('admin.kasir.toggle', $kasir) }}" class="inline" onsubmit="return confirm('Are you sure?')">
                                @csrf @method('PATCH')
                                <button type="submit" class="text-xs capitalize
                ...
                @empty
                    <tr><td colspan="4" class="p-8 text-center text-sm text-muted-foreground">No kasir accounts yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<!-- Create Modal -->
<div id="create-kasir" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50" onclick="this.classList.add('hidden')">
    <div class="relative w-full max-w-md rounded-xl bg-card p-6 shadow-lg" onclick="event.stopPropagation()">
        ...
        <form method="POST" action="{{ route('admin.kasir.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <input name="email" required maxlength="255" class="mt-1 w-full h-9 rounded-md border border-input bg-transparent px-3 text-sm outline-none transition-colors focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50">
            </div>
            <div>
                <label class="text-sm font-medium">Password</label>
                <input name="password" required min="6" class="mt-1 w-full h-9 rounded-md border border-input bg-transparent px-3 text-sm outline-none transition-colors focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50">
            </div>
            <div class="flex justify-end gap-2">
                <button type="button" onclick="document.getElementById('create-kasir').classList.add('hidden')" class="rounded-md border text-sm font-medium h-9 px-4 hover:bg-accent/50">Cancel</button>
