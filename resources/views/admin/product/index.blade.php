...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <button data-modal="create-product"
            ...
            <tbody>
                ...
                <tr class="border-t last:border-0 hover:bg-accent/30">
                    ...
                    <td class="p-4 text-right">
                        <form method="POST" action="{{ route('admin.products.update', $prod) }}"
                            class="inline-flex items-center gap-2">
                            @csrf @method('PUT')
                            <input type="text" name="name" value="{{ $prod->name }}"
                                class="h-8 w-32 rounded-md border border-input bg-transparent px-1 text-sm">
                            <button type="submit"
                                class="text-xs bg-primary text-primary-foreground px-2 py-1 rounded hover:opacity-90">Save</button>
                        </form>
                        <form method="POST" action="{{ route('admin.products.destroy', $prod) }}" class="inline"
                ...
                @empty
                <tr>
                    <td colspan="5" class="p-8 text-center text-sm text-muted-foreground">No products yet.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div id="create-product" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50"
    ...
    <div class="relative w-full max-w-md rounded-xl bg-card p-6 shadow-lg" onclick="event.stopPropagation()">
        ...
        <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data"
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="2"
                        class="mt-1 w-full h-24 rounded-md border border-input bg-transparent px-3 text-sm outline-none resize-none transition-colors focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50"></textarea>
                </div>
                <div>
                    <label class="text-sm font-medium">Image</label>
                    <input type="file" name="image"
                        class="mt-1 w-full h-9 rounded-md border border-input bg-transparent px-3 text-sm">
                </div>
                <div>
            ...
            <div class="flex justify-end gap-2">
                <button type="button" onclick="document.getElementById('create-product').classList.add('hidden')"
