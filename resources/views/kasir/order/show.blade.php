...
<body class="min-h-screen bg-background text-foreground">
    ...
    <main class="mx-auto max-w-3xl px-4 py-6 space-y-6">
        @if(session('success'))
            <div class="rounded-md border border-green-300 bg-green-50 p-3 text-sm text-green-800">{{ session('success') }}</div>
        @endif
        @if($errors->any())
        ...
        <div class="space-y-3">
            @if($order->status !== \App\Enums\OrderStatus::Paid)
                ...
                <div class="rounded-xl border bg-card shadow-sm p-4">
                    ...
                    <div>
                        <button type="button" data-open-payment="cash" class="w-full rounded-md bg-primary text-primary-foreground text-sm font-medium h-9 hover:opacity-90 transition-opacity">
                            Pay
                        </button>
                    </div>
                    <form id="payment-submit-form" method="POST" action="{{ route('kasir.order.pay', $order->id) }}" class="hidden">
                        @csrf
            ...
            @else
                <div class="rounded-xl border bg-green-50 p-4 text-center text-sm text-green-800 space-y-2">
                    <p>Payment processed using <span class="font-semibold uppercase">{{ $order->payment_method?->value }}</span>. Table {{ $order->table->number }} is now available.</p>
                    <a href="{{ route('kasir.order.receipt', $order) }}" class="inline-flex rounded-md bg-green-700 px-3 py-2 text-xs font-semibold text-white hover:bg-green-800">
                        View / Print receipt
                    </a>
                </div>
            @endif
    ...
    @if($order->status === \App\Enums\OrderStatus::Served)
        <dialog id="payment-confirmation-dialog" class="w-full max-w-md rounded-2xl border bg-card p-0 shadow-2xl backdrop:bg-black/50">
            <div class="p-6">
                <div class="flex items-start justify-between gap-4">
                    ...
                    <button type="button" data-close-payment class="rounded-lg p-2 text-muted-foreground hover:bg-muted" aria-label="Tutup">&times;</button>
                </div>

                <div class="mt-5 rounded-xl border bg-muted/30 p-4 text-sm">
                    <div class="flex justify-between gap-3"><span>{{ $order->customer_name }}</span><span>Table {{ $order->table->number ?? '-' }}</span></div>
                    <div class="mt-3 flex justify-between border-t pt-3 font-semibold"><span>Total</span><span>Rp {{ number_format($order->total_price, 0, ',', '.') }}</span></div>
                </div>

                <div data-payment-step="method">
                    <p class="mt-5 text-sm text-muted-foreground">Select the payment method received.</p>
                    <div class="mt-3 grid grid-cols-2 gap-3">
                        <button type="button" data-select-payment="cash" class="rounded-xl border p-4 text-left text-sm hover:bg-muted"><span class="block font-semibold">Cash</span><span class="mt-1 block text-xs text-muted-foreground">Cash payment</span></button>
                        <button type="button" data-select-payment="qris" class="rounded-xl border p-4 text-left text-sm hover:bg-muted"><span class="block font-semibold">QRIS</span><span class="mt-1 block text-xs text-muted-foreground">Manual confirmation</span></button>
                    </div>
                    <p data-selected-payment class="mt-3 text-sm text-muted-foreground">Method: Cash</p>
                    <div class="mt-6 flex justify-end gap-3"><button type="button" data-close-payment class="rounded-lg border px-4 py-2 text-sm font-medium hover:bg-muted">Cancel</button><button type="button" data-continue-payment class="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:opacity-90">Continue</button></div>
                </div>

                <div data-payment-step="confirmation" hidden>
                    <div class="mt-5 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900"><p>This order will be marked as <strong>paid</strong> using <strong data-confirm-payment-method>Cash</strong>.</p><label data-qris-confirmation class="mt-4 hidden items-start gap-3"><input type="checkbox" data-qris-checkbox class="mt-0.5 rounded border-amber-500 text-primary focus:ring-primary"><span>I have received and manually verified the QRIS payment.</span></label></div>
                    <div class="mt-6 flex justify-between gap-3"><button type="button" data-back-payment class="rounded-lg border px-4 py-2 text-sm font-medium hover:bg-muted">Back</button><button type="button" data-submit-payment class="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:opacity-90">Confirm payment</button></div>
                </div>
            </div>
        </dialog>
