                ...
                @foreach(['pending', 'preparing', 'ready', 'served', 'paid'] as $i => $state)
                    <li class="flex flex-1 items-center">
                        <div class="flex flex-col items-center gap-2" data-status="{{ $state }}">
                            <div class="status-dot size-5 rounded-full {{ $order->status->value === $state ? 'bg-primary' : 'bg-muted' }}"></div>
                            <span class="text-xs capitalize {{ $order->status->value === $state ? 'font-medium text-foreground' : 'text-muted-foreground' }}">{{ $state }}</span>
                        </div>
                        @if(!$loop->last)
