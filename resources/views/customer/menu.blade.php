...
<body class="min-h-screen bg-background text-foreground">
    <header class="sticky top-0 z-40 border-b bg-background/95 backdrop-blur">
        <div class="mx-auto flex h-14 items-center justify-between px-4 max-w-5xl">
            ...
            <a href="{{ route('meja.checkout') }}" class="relative rounded-md border px-3 py-1.5 text-sm bg-primary text-primary-foreground">
                Cart
                @php($cart = session('cart', []))
                @if(count($cart) > 0)
                    <span class="absolute -top-2 -right-2 flex size-5 items-center justify-center rounded-full bg-destructive text-white text-xs">
                ...
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($category->products as $product)
                        <div class="rounded-xl border bg-card shadow-sm p-4 flex flex-col gap-2">
                            <div class="aspect-square w-full rounded-lg bg-accent flex items-center justify-center text-muted-foreground text-sm">
                                {{ $product->name[0] }}
                            </div>
                            <div>
                            ...
                            <div class="flex items-center justify-between mt-auto pt-2">
                                ...
                                <form action="{{ route('meja.cart.add', $table->qr_token) }}" method="POST" class="flex items-center gap-2">
                                    @csrf
                                    <input type="hidden" name="product_id" value="{{ $product->id }}">
                                    <select name="quantity" class="h-8 w-14 rounded-md border border-input bg-transparent px-1 text-sm">
                                        @for($i = 1; $i <= 10; $i++)
                                            <option value="{{ $i }}" {{ $i === 1 ? 'selected' : '' }}>{{ $i }}</option>
                                        @endfor
                                    </select>
                                    <button type="submit" class="rounded-md bg-primary text-primary-foreground px-3 py-1.5 text-xs font-medium hover:opacity-90 transition-opacity">Add</button>
                                </form>
                            </div>
                        </div>
                    @endforeach
