...
<body class="min-h-screen bg-background text-foreground">
    <header class="border-b">
        <div class="mx-auto flex h-14 items-center px-4 max-w-5xl">
            <a href="{{ url()->previous() }}" class="text-sm text-muted-foreground hover:text-foreground">&larr; Back to
                menu</a>
        </div>
    </header>
    ...
    <main class="mx-auto max-w-3xl px-4 py-6 space-y-6">
        ...
        <div class="rounded-xl border bg-card shadow-sm">
            ...
            <div class="flex items-center justify-between p-4 border-b last:border-b-0">
                <div>
                    ...
                    <p class="text-xs text-muted-foreground">Rp {{ number_format($item['price'], 0, ',', '.') }} &times;
                        {{ $item['quantity'] }}</p>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <form action="{{ route('meja.cart.remove', [session('table_qr'), $item['product_id']]) }}"
                        ...
                        @method('DELETE')
                        <button type="submit"
                            class="text-xs text-destructive hover:underline cursor-pointer">Remove</button>
                    </form>
                </div>
            </div>
            @endforeach
        ...
        <div class="flex justify-between items-center rounded-xl border bg-card shadow-sm p-4">
            ...
            <span class="font-bold text-lg">Rp {{ number_format(collect($cart)->sum(fn($i) => $i['price'] *
                $i['quantity']), 0, ',', '.') }}</span>
        </div>

        <form action="{{ route('meja.checkout.store') }}" method="POST"
            class="rounded-xl border bg-card shadow-sm p-4 space-y-4">
            @csrf
            <div class="space-y-2">
